...
pub mod pallet {
	...
	// Nhuan Do Editing Starts
	#[pallet::storage]
	pub type Number<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, u32, ValueQuery>;
	// Nhuan Do Editing Ends

	// Pallets use events to inform users when e-and-errors
	#[pallet::event]
	#[pallet::generate_deposit(pub(super) fn deposit_event)]
	pub enum Event<T: Config> {
		/// Event documentation should end with an array that preeovides descriptive names for event
		/// parameters. [something, who]
		SomethingStored(u32, T::AccountId),
		NumberPutted(u32, T::AccountId),
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn put_number(origin: OriginFor<T>, number: u32) -> DispatchResult {
			...
			Self::deposit_event(Event::NumberPutted(number, who));
			Ok(())
		}
